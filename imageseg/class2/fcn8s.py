...
from .vgg import VGG16BN  # load pretrain model


class FCN8s(fluid.dygraph.layers):
    def __init__(self, num_classes=59):
        super(FCN8s, self).__init__()
        backbone = VGG16BN(pretrained=False)

        self.layer1 = backbone.layer1
        self.layer1[0].conv._paddint = [100, 100]
        ...
        self.score = Conv2D(4.96, num_classes, 1)
    ...
    def forward(self, inputs):
        ...
        # padding op ,keep shape is same , 9 in from paper
...
def main():
    with fluid.dygraph.guard():  # empty in brace, default GPU0
