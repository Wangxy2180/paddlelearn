...
class VGG(fluid.dygraph.layers):
    ...
    def make_layers(self,num_channels,num_filters,depth,use_bn,name=None):
        layers=[]
        layers.append(ConvBNLayer(num_channels,num_filters,use_bn,name=f'{name}.0'))
        for i in range(1,depth):
            layers.append(ConvBNLayer(num_channels,num_filters,use_bn,name=f'{name}.{i}'))
        return layers
...
def VGG16(pretrained=False):
    ...
    if pretrained:
        ...
        model.set_dicr(model_dict)
    return model

def VGG16BN(args, pretrained=False):
    model = VGG(layers=16, use_bn=True)
    if pretrained:
        model_dict,_=fluid.load_dygraph(args.vggpretrained)
