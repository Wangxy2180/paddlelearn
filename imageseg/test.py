...
class CenterCrop(object):
    def __init__(self, size):
        assert type(size) in [int, tuple], "CHECK SIZE TYPE!"   # 断言确认size类型为int或tuple，int型指定输出图像为正方形，tuple类型可以指定
...
def main():
    ...
    image_path = os.path.join(current_path, "dummy_data/JPEGImages/2008_000064.jpg")    # 搞不清楚当前工作目录的相对路径的同学，建议写文件地
    ...
    # transform = CenterCrop(size=crop_size)    # 输出10张一致的从中间裁剪的大小为crop_size的图像，注意crop_size应小于min(h,w), 否则操作越界
    ...
    for i in range(10):
        ...
        # TODO: save image
        save_path = os.path.join(current_path, 'class1/save')
...
if __name__ == "__main__":
    ...
    a0 = cv2.imread('/class1/save/0.jpg')
    cv2.imshow('0', a0)
    cv2.waitKey(50000)
    # img.imread()